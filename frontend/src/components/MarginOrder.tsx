...
export function MarginOrder() {
  ...
  const handlePlaceOrder = () => {
    ...
    writeContract({
      ...
      args: [fromToken.address, toToken.address, selectedFee, isShort, leverage, amount, 0, 0],
    });
  };
  ...
  return (
    <div className="space-y-4">
      <div className="p-4 border rounded-lg bg-gray-50">
        <label className="block text-xs font-medium text-gray-500">Collateral</label>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label htmlFor="positionType" className="block text-sm font-medium text-gray-700">Position</label>
          <select id="positionType" name="positionType" className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md" value={isShort ? 'Short' : 'Long'} onChange={(e) => setIsShort(e.target.value === 'Short')}>
            <option>Long</option>
            <option>Short</option>
          </select>
        </div>
        <div>
          <label htmlFor="leverage" className="block text-sm font-medium text-gray-700">Leverage: {leverage}x</label>
          <input type="range" id="leverage" name="leverage" min="1" max="5" step="1" className="mt-1 w-full" value={leverage} onChange={(e) => setLeverage(parseInt(e.target.value, 10))} />
        </div>
      </div>
      ...
      <div className="pt-4">
        {needsApproval ? (
          ...
          <button type="button" onClick={handlePlaceOrder} disabled={isPending || !fromAmount} className="w-full inline-flex items-center justify-center px-4 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:bg-indigo-400">
            {isPending ? 'Opening Position...' : 'Open Margin Position'}
          </button>
        )}
      </div>
