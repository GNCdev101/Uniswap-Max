...
export function StopLossOrder() {
  ...
  const handlePlaceOrder = () => {
    if (!fromAmount || !account || !stopLossPrice) return;
    const amount = parseUnits(fromAmount, fromToken.decimals);
    const price = parseUnits(stopLossPrice, 18);

    writeContract({
      address: MARKET_CONTRACT_ADDRESS,
      abi: Market_ABI,
      functionName: 'openPosition',
      args: [fromToken.address, toToken.address, selectedFee, false, 1, amount, 0, price],
    });
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="p-4 border rounded-lg bg-gray-50">
        <label htmlFor="stopLossPrice" className="block text-xs font-medium text-gray-500">Stop Loss Price ({toToken.symbol} per {fromToken.symbol})</label>
        <div className="mt-1">
          <input type="number" name="stopLossPrice" id="stopLossPrice" className="text-2xl bg-transparent focus:outline-none block w-full" placeholder="0.0" value={stopLossPrice} onChange={(e) => setStopLossPrice(e.target.value)} />
        </div>
      </div>
      ...
      <div className="pt-4">
        {needsApproval ? (
          ...
          <button type="button" onClick={handlePlaceOrder} disabled={isPending || !fromAmount || !stopLossPrice} className="w-full inline-flex items-center justify-center px-4 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:bg-indigo-400">
            {isPending ? 'Placing Order...' : 'Place Stop Loss Order'}
          </button>
        )}
      </div>
